...
export function addCookie(
  ...
  nickname: string,
  vipType: number,
  persistPolicy: 'room' | 'server' = 'server',
): void {
...
export function cleanupRoom(roomId: string): void {
  ...
  platformAuthRepo.cleanupRoom(roomId)
}

export function restoreRoom(roomId: string): void {
  const restored = platformAuthRepo.loadRoom(roomId)
  if (restored.length === 0) return

  const pool = getRoomPool(roomId)
  for (const entry of restored) {
    const entries = getPlatformEntries(roomId, entry.platform)
    const idx = entries.findIndex((e) => e.cookie === entry.cookie || e.userId === entry.userId)
    if (idx !== -1) entries.splice(idx, 1)
    entries.push({
      cookie: entry.cookie,
      userId: entry.userId,
    ...
    pool.set(entry.platform, entries)
  }

  logger.info(`Auth: restored ${restored.length} persisted cookies for room ${roomId}`)
}

export function clearPersistedUserCookies(roomId: string, userId: string): void {
...
export function hasVipCookie(platform: MusicSource, roomId: string): boolean {
  const pool = roomCookiePool.get(roomId)
  const entries = pool?.get(platform) ?? []
  return entries.some((entry) => entry.vipType > 0)
}
