...
export function scheduleDeletion(roomId: string, io?: TypedServer): void {
  ...
  const timer = setTimeout(() => {
    ...
    if (r && !r.permanent && r.users.every((user) => user.online === false)) {
...
export function destroyRoom(roomId: string, io?: TypedServer): boolean {
  ...
  cancelDeletionTimer(roomId)
  const socketIds = roomRepo.getSocketIdsForRoom(roomId)

  if (io) {
    io.to(roomId).emit(EVENTS.ROOM_DISSOLVED, { roomId })
  }
  ...
  logger.info(`Room ${roomId} destroyed`, { roomId })
  return true
}
