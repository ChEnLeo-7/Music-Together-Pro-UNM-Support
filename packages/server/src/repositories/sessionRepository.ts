...
export function createSessionRepository(database: Database) {
  ...
  const revokeAllExcept = database.prepare(
  ...
  return {
    ...
    revoke(sessionId: string, now = Date.now()): void {
      revoke.run(now, sessionId)
    },
    revokeAllForUser(userId: string, now = Date.now()): void {
      revokeAll.run(now, userId)
    },
    revokeAllExcept(userId: string, sessionId: string, now = Date.now()): void {
      revokeAllExcept.run(now, userId, sessionId)
    },
