...
export function registerAuthController(io: TypedServer, socket: TypedSocket) {
  ...
  socket.on(EVENTS.AUTH_SET_COOKIE, async (data) => {
    try {
      ...
      const persistPolicy = data.persist === false ? 'room' : 'server'
      const mapping = getSocketMapping(socket.id)
      ...
      if (infoResult.ok) {
        ...
        if (mapping && mapping.roomId) {
          authService.addCookie(mapping.roomId, platform, mapping.userId, cookie, '手动登录', 0, persistPolicy)
        }
        socket.emit(EVENTS.AUTH_SET_COOKIE_RESULT, {
          success: true,
  ...
  socket.on(EVENTS.AUTH_SET_PERSISTENCE, (data) => {
    try {
      const mapping = getSocketMapping(socket.id)
      if (!mapping) return
      if (data?.persist === false) {
        authService.clearPersistedUserCookies(mapping.roomId, mapping.userId)
      }
    } catch (err) {
      logger.error('AUTH_SET_PERSISTENCE handler error', err, { socketId: socket.id })
    }
  })
...
function broadcastAuthStatus(io: TypedServer, socket: TypedSocket, mapping: { roomId: string; userId: string }) {
  ...
  if (room?.currentTrack) {
    room.currentTrack.availableStreamQualities = undefined
    io.to(mapping.roomId).emit(EVENTS.ROOM_STATE, roomService.toPublicRoomState(room))
  }
