...
import { mergeLrcTextByTime } from '@/lib/lyricTimeline'
...
export function useLyric() {
  ...
  const lyricKeyRef = useRef<string | null>(null)
  const lyricCacheRef = useRef<{
    key: string
    lyric: string
    tlyric: string
    ttmlLines: AMLLLyricLine[] | null
  } | null>(null)
  const lyricRequestRef = useRef<Promise<void> | null>(null)
  ...
  const fetchLyric = useCallback(
    (track: Track) => {
      ...
      if (lyricCacheRef.current?.key === lyricKey) {
        ...
        setLyric(lyricCacheRef.current.lyric, lyricCacheRef.current.tlyric)
        setLyricLoading(false)
        lyricKeyRef.current = lyricKey
      ...
      const request = (async () => {
        ...
        if (ttmlEnabled && folder) {
          try {
            ...
            const timeoutSignal =
              typeof AbortSignal?.timeout === 'function' ? AbortSignal.timeout(TTML_TIMEOUT_MS) : null
        ...
        setLyricLoading(false)
        const lyricState = usePlayerStore.getState()
        lyricCacheRef.current = {
          key: lyricKey,
          lyric: lyricState.lyric,
          tlyric: lyricState.tlyric,
          ttmlLines: lyricState.ttmlLines,
        }
      })()
      lyricRequestRef.current = request
      void request.finally(() => {
        if (lyricRequestRef.current === request) lyricRequestRef.current = null
      })
      return request
    },
