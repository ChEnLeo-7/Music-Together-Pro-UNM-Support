...
import type { AudioEngine } from './useHowl'
    ...
    const onSeek = (data: { playState: ScheduledPlayState }) => {
      ...
      scheduledTimerRef.current = setTimeout(() => {
        ...
        const suppressLocalEcho =
          Date.now() < playerState.suppressNextRemoteSeekUntil &&
          playerState.suppressNextRemoteSeekTarget !== null &&
          Math.abs(playerState.suppressNextRemoteSeekTarget - data.playState.currentTime) < 0.25
        if (howlRef.current) {
          if (!suppressLocalEcho) howlRef.current.seek(data.playState.currentTime)
