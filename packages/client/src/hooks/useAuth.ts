...
export function useAuth() {
  ...
  const platformStatus = useAuthStore((s) => s.platformStatus)
  const myStatus = useAuthStore((s) => s.myStatus)
  const statusLoaded = useAuthStore((s) => s.statusLoaded)
  ...
  const setCookie = useCallback(
    (platform: MusicSource, cookie: string, persist = true) => {
