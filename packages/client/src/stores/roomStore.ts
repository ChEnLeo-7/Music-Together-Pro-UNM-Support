...
export const useRoomStore = create<RoomStore>((set) => ({
  ...
  setRoom: (room) =>
    set((state) => {
      if (!room) return { room: null, currentUser: null }
      const nextRoom: RoomState = {
        ...room,
        queue: room.queue ?? state.room?.queue ?? [],
      }
      return { room: nextRoom, currentUser: deriveCurrentUser(nextRoom) }
    }),

  updateRoom: (partial) =>
    set((state) => {
      if (!state.room) return {}
      const room = { ...state.room, ...partial, queue: partial.queue ?? state.room.queue }
  ...
  markUserOffline: (userId) =>
    set((state) => {
      ...
      const room = {
        ...state.room,
        users: state.room.users.map((u) => (u.id === userId ? { ...u, online: false } : u)),
      }
      return { room, currentUser: deriveCurrentUser(room) }
    }),
  ...
  updateUserProfile: (userId, profile) =>
    set((state) => {
      ...
      const room = {
        ...state.room,
        users: state.room.users.map((u) => (u.id === userId ? { ...u, ...profile } : u)),
      }
      return { room, currentUser: deriveCurrentUser(room) }
    }),
