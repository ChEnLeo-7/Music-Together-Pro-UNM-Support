...
20. **后台标签页可靠切歌**：服务端会根据权威播放时间在歌曲结束后兜底推进队列；网页从后台恢复时会立即校准进度，不再依赖房主标签页持续处于前台
21. **播放后自动清理队列**：房主可开启播放完毕后自动从播放列表删除歌曲；队列清空后自动停止播放
...
从 [GitHub Releases](https://github.com/ChEnLeo-7/Music-Together-Pro-UNM-Support/releases/latest) 下载 `v0.10.6` 或更新版本的 APK 并安装。首次启动时输入 Music Together 服务端地址，例如 `https://music.example.com`；可信局域网调试也可使用 `http://192.168.1.10:3001`。

Android App 使用 Media3 前台播放服务，支持后台/锁屏播放和系统媒体控制，并支持播放服务端保存的自定义媒体。`v0.10.6` 增加服务端歌曲结束兜底与网页后台恢复同步，避免房主网页标签页进入后台后阻塞整个房间切换下一首。
